...
- _set_ `products(value: IProduct[])` - установить список товаров (вызывает событие - обновление списка товаров)
- _get_ `products(): IProduct[]` - получить список товаров

### 3. Модель корзины (`BasketModel`)

Реализует логику работы с корзиной. В неё нельзя добавлять одинаковые товары.

**Поля**:

- `_products: Map<string, IBasketProductData>` — словарь уникальных товаров в корзине

**Методы**:

- _get_ `products(): IBasketProduct[]` — получить список товаров в корзине
- _get_ `totalPrice(): number` — получить общую стоимость товаров в корзине
- `add(product: IBasketProduct)`— добавить товар в корзину (вызывает событие - изменение корзины)
- `remove(productId: string)` — удалить товар из корзины (вызывает событие - изменение корзины)
- `has(productId: string): boolean` — проверить наличие товара в корзине
- `getIndex(productId: string): number` — получить индекс элемента в корзине
...
- _get_ `userData(): IUserData` - получить данные пользователя
...
1. **View → Presenter**:
   ...
   - `CardPreviewView` генерирует событие `ViewStates.basketProductAdd` с id товара

2. **Presenter → Model**:

   - Презентер ловит событие `ViewStates.basketProductAdd`
   - Вызывает метод `basketModel.add()` для добавления товара в корзину

3. **Model → Presenter**:

   - `BasketModel` добавляет товар в `_products`
   - Генерирует событие `ModelStates.basketChange`

4. **Presenter → View**:

   - Презентер ловит событие `ModelStates.basketChange`
   - Получает обновленные данные из `basketModel`
