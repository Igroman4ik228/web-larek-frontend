...
import {
	BasketProductAddEvent,
	BasketProductRemoveEvent,
	CardSelectEvent,
...
const basketView = new BasketView(ensureElement<HTMLElement>('.basket'), {
	onClick: () => {
		// Валидация корзины, чтобы нельзя было заказать пустую
		if (basketModel.isValid) events.emit(ViewStates.basketSubmit);
	},
});
...
const contactView = new ContactView(
	cloneTemplate(TEMPLATES.orderContacts),
	events
);
const successView = new SuccessView(cloneTemplate(TEMPLATES.success), {
...
events.on<SuccessOpenEvent>(
	ModelStates.successOpen,
	(event: SuccessOpenEvent) => {
		modalView.render({
...
events.on<BasketProductRemoveEvent>(
	ViewStates.basketProductRemove,
	(event: BasketProductRemoveEvent) => {
...
events.on<BasketProductAddEvent>(
	ViewStates.basketProductAdd,
	(event: BasketProductAddEvent) => {
		basketModel.add({
			id: event.product.id,
...
events.on<FormFieldChangeEvent>(
	/^view:order\..*-change/,
...
events.on(ViewStates.orderContactsSubmit, () => {
	...
	larekApi
		...
		.then((result) => {
			...
			events.emit<SuccessOpenEvent>(ModelStates.successOpen, {
...
function renderCard(item: IProduct): HTMLElement {
	const cardView = new CardView(cloneTemplate(TEMPLATES.catalog), {
		onClick: () =>
			events.emit<CardSelectEvent>(ViewStates.cardSelect, {
				productId: item.id,
			}),
	});
	...
	const cardPreviewView = new CardPreviewView(
		cloneTemplate(TEMPLATES.cardPreview),
		{
			onClick: () => {
				...
				if (catalogModel.getProduct(item.id).price === null) return;

				if (isInBasket)
					events.emit<BasketProductRemoveEvent>(
						ViewStates.basketProductRemove,
						{ productId: item.id }
					);
				else
					events.emit<BasketProductAddEvent>(ViewStates.basketProductAdd, {
						product: item,
					});
